...
void UWidget_KeyRemapScreen::RequestDeactivateWidget(TFunction<void()> PesDeactivateCallback)
{
	//延迟一帧以确保输入能被正确处理
	FTSTicker::GetCoreTicker().AddTicker(
		FTickerDelegate::CreateLambda(
			[PesDeactivateCallback,this](float DeltaTime)->bool
			{
				PesDeactivateCallback();
