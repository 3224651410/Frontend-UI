	...
	UFrontendUISubsystem::Get(this)->PushConfirmScreenToModalStackAynsc(
		...
		[this](EConfirmScreenButtonType ClickedButtonType)
		{
			...
			for (UListDataObject_Base* DataToReset : ResettableDataArray)
			{
				...
				if (DataToReset->TryResetBackToDafalutValue())
				{
				...
				else
				{
					...
					Debug::Print(DataToReset->GetDataDisplayName().ToString() + TEXT(" was failed to reset"));
				}
			}
...
void UWidget_OptionsScreen::OnOptionsTabSelected(FName TabId)
{
	...
	for (UListDataObject_Base* FoundListSourceItem : FoundListSourceItems)
	{
		...
		if (FoundListSourceItem->CanResetBackToDafaultValue())
		{
...
void UWidget_OptionsScreen::OnListViewItemHovered(UObject* InHoveredItem, bool bWasHovered)
{
	...
	if (bWasHovered)
	{
		DetailsView_ListEntryInfo->UpdataDetailsViewInfo(
...
void UWidget_OptionsScreen::OnListViewListDataModified(UListDataObject_Base* ModifiedData, EOptionsListDataModifyReason ModifiedReason)
{
	...
	if (ModifiedData->CanResetBackToDafaultValue())
	{
