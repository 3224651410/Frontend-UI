...
void UWidget_OptionsDetailsView::UpdataDetailsViewInfo(UListDataObject_Base* InDataObject, const FString& InEntryWidgetClassName)
{
	if (!InDataObject)
	{
		return;
	...
	CommonRichText_Discription->SetText(InDataObject->GetDescriptionRichText());
...
void UWidget_OptionsDetailsView::ClearDetailsViewInfo()
{
	...
	CommonLazyImage_Description->SetVisibility(ESlateVisibility::Collapsed);
	CommonRichText_Discription->SetText(FText::GetEmpty());
