...
void UWidget_ListEntry_String::OnRotatorValueChanged(int32 Value, bool bUserInitiated)
{
	...
	UCommonInputSubsystem* CommonInputSybsystem = GetInputSubsystem();
	
	if (!CommonInputSybsystem || !bUserInitiated)
	{
		return;
	}

	if (CommonInputSybsystem->GetCurrentInputType() == ECommonInputType::Gamepad)
	{
