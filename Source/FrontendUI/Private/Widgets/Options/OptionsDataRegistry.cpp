...
#define GET_DESCRIPTION(InKey) LOCTABLE("/Game/UI/StringTables/ST_OptionsScreenDescriprion.ST_OptionsScreenDescriprion", InKey)
...
void UOptionsDataRegistry::InitGameplayCollectionTab()
{
	...
	//Game Difficulty
	{
		...
		GameDifficulty->SetDescriptionRichText(FText::FromString(TEXT("Adjusts the difficult of the game experience.\n\n<Bold>Easy:</> Focuses on the story experience. Privodes the most relaxing combat.\n\n<Bold>Normal:</> Offers slightly harder combat experience\n\n<Bold>Hard:</> Offers a much more challenging combat experience\n\n<Bold>Very Hard:</> Provides the most challenging combat experience. Not recommended for first play through.")));
	...
	//Test Item
	{
		...
		TestItem->SetDescriptionRichText(FText::FromString(TEXT("This is a display image.")));
...
void UOptionsDataRegistry::InitAudioCollectionTab()
{
	...
	//Volume Category
	{
		...
		{
			UListDataObject_Scalar* SoundEffectsVolume = NewObject<UListDataObject_Scalar>();
			SoundEffectsVolume->SetDataID(FName("SoundEffectsVolume"));
			SoundEffectsVolume->SetDataDisplayName(FText::FromString(TEXT("Sound Effects Volume")));
			SoundEffectsVolume->SetDescriptionRichText(FText::FromString(TEXT("This is description for Sound Effects Volume")));
			SoundEffectsVolume->SetDisplayValueRange(TRange<float>(0.f, 1.f));
			SoundEffectsVolume->SetOutputValueRange(TRange<float>(0.f, 2.f));
			SoundEffectsVolume->SetSliderStepSize(0.01f);
			SoundEffectsVolume->SetDefaultValueFromString(LexToString(1.f));
			SoundEffectsVolume->SetDisplayNumericType(ECommonNumericType::Percentage);
			SoundEffectsVolume->SetNumberFormattingOptions(UListDataObject_Scalar::NoDecimal());
			SoundEffectsVolume->SetDataDynamicGetter(MAKE_OPTIONS_DATA_CONTROL(GetSoundEffectsVolume));
			SoundEffectsVolume->SetDataDynamicSetter(MAKE_OPTIONS_DATA_CONTROL(SetSoundEffectsVolume));
			SoundEffectsVolume->SetShouldApplySettingsImmediately(true);

			VolumeCategoryCollection->AddChildListData(SoundEffectsVolume);
		}
	...
	//Sound Categoty
	{
		...
		//Use HDR Audio
		{
			...
			SoundCollection->AddChildListData(UseHDRAudioMode);
		}
	}
...
void UOptionsDataRegistry::InitVideoCollectionTab()
{
	...
	//Display Category
	{
		...
		//Window Mode
		{
			...
			WindowMode->SetDataDisplayName(FText::FromString(TEXT("Display")));
		...
		//Screen Resolution
		{
			...
			ScreenResolution->SetDataDisplayName(FText::FromString(TEXT("Window Mode")));
	...
	//Graphics Category
	{
		...
		//Display Gamma
		{
			...
			DisplayGamma->SetOutputValueRange(TRange<float>(1.7f, 2.7f));//Unreal 中默认值为2.2f
	...
	//Advanced Graphics Category
	{
		...
		AdvancedGraphicsCategoryCollection->SetDataDisplayName(FText::FromString(TEXT("Advanced Graphicsn")));
...
void UOptionsDataRegistry::InitControlCollectionTab(ULocalPlayer* InOwningLocalPlayer)
{
	...
	//Keyboare Mouse Category
	{
		UListDataObject_Collection* KeyboardMousCategoryCollection = NewObject<UListDataObject_Collection>();
		KeyboardMousCategoryCollection->SetDataID(FName("KeyboardMousCategoryCollection"));
		KeyboardMousCategoryCollection->SetDataDisplayName(FText::FromString(TEXT("Keyboard & Mouse")));

		ControlTabCollection->AddChildListData(KeyboardMousCategoryCollection);
				...
				for (const TPair<FName, FKeyMappingRow>& MappingRowPair : MappableKeyProfile->GetPlayerMappingRows())
				{
					for (const FPlayerKeyMapping& KeyMapping : MappingRowPair.Value.Mappings)
					{
						if (MappableKeyProfile->DoesMappingPassQueryOptions(KeyMapping, KeyboardMouseOnly))
						{
							...
							KeyboardMousCategoryCollection->AddChildListData(KeyRemapDataObject);
						}
					}
				}
			}
