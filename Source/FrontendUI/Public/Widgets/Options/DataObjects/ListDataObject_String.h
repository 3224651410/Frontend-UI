...
protected:
	...
	virtual bool CanResetBackToDafaultValue() const override;
	virtual bool TryResetBackToDafalutValue() override;
...
public:
	...
	void AddEnumOption(EnumType InEnumOption, const FText& InDisplayText)
	{
		...
		const FString ConveretedString = StaticEnumOption->GetNameStringByValue(InEnumOption);

		AddDynamicOption(ConveretedString, InDisplayText);
	}
	...
	void SetDefaultValueFromEnumOption(EnumType InEnumOption)
	{
		...
		SetDefaultValueFromString(ConveretedString);
	}
};
